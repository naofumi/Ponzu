...
module Import
  ...
  class UminFile
    ...
    # because our parser handles CSV rows as-is.
    # (this allows us to test against CSV text strings)
    #
    # However, we need to use CSV.foreach instead of File.gets
    # because line-breaks may occur inside a field. If we simply 
    # used File.gets to get each CSV line, we would prematurely
    # terminate lines if there were any embedded line-breaks.
    #
    # Therefore, we first use CSV.foreach to get correctly get
    # each CSV line, and then reconstitute the CSV string
    # from the `row` argument sent to the CSV.foreach block.
    #
    # Before 2014.4.4 (JSDB2014), we used UTF-8 encoded CSV files
    # which were generated by exporting Shift-JIS CSV files from Excel
    # and converting them to UTF-8 with BBEdit. This causes problems
    # because Shift-JIS only covers a small subset of Japanese characters.
    # We now use files exported from Excel with the "UTF-16 Unicode text" 
    # option.
    #
    # Notes on Excel UTF-16 text format.
    # When exporting in UTF-16 from Excel, Excel uses a UTF-16LE format with
    # a BOM. The line endings are \r\n, even on a Mac, and I presume that they
    # are the same on Windows as well.
